...
gallery.innerHTML = markup;
 ...
 const galleryModal = gallery.addEventListener('click', (event) => {
  event.preventDefault();
  const clickedImage = event.target.dataset.source;
  if (clickedImage) { console.log(clickedImage) };
  
   const instance = basicLightbox.create(` <img src="${clickedImage}" width="360" height="200">`,
{ onShow: () => { window.addEventListener('keydown', onEsc)},
onClose: () => { window.removeEventListener('keydown', onEsc) },});
   
  const onEsc = (event) => { console.log(event.code);
